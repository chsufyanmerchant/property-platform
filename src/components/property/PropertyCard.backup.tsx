...
export default function PropertyCard({ property, isFavorited, onToggleFavorite }: PropertyCardProps) {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-surface shadow-card transition-all duration-300 hover:-translate-y-1 hover:border-teal/20 hover:shadow-card-hover">
      ...
      />
      {/* Placeholder art — original gradient + kashi pattern, no stock photography */}
<div
  className={cn(
    "relative aspect-[4/3] overflow-hidden bg-gradient-to-br",
    gradientForSeed(property.id)
  )}
>        <TilePattern id={property.id} className="absolute inset-0 h-full w-full text-white/10" />
          ...
          {onToggleFavorite && (
            <button
              ...
              className={cn(
                ...
                isFavorited ? "bg-white text-red-500" : "bg-white/90 text-ink hover:text-red-500"
              )}
            >
              <Icon name="heart" className="h-4 w-4" fill={isFavorited ? "currentColor" : "none"} />
            </button>
          )}
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        <div className="mb-1.5 flex items-baseline justify-between gap-2">
          <p className="text-lg font-bold text-ink">{formatPKR(property.price, property.purpose)}</p>
          <span className="shrink-0 rounded-full bg-canvas px-2 py-0.5 text-xs font-medium text-ink-soft">
            {typeLabel[property.type]}
          </span>
        </div>

        <h3 className="mb-1 truncate text-sm font-semibold text-ink">{property.title}</h3>
        ...
        <div className="mt-auto flex items-center justify-between border-t border-line pt-3 text-xs text-ink-muted">
          <span className="flex items-center gap-1.5 truncate">
            {property.isVerified && <Icon name="shieldCheck" className="h-3.5 w-3.5 shrink-0 text-teal" />}
